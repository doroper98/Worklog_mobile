...
>
> **작성 기준일.** 2026-07-27 (rev.2 — 데스크탑 무영향 검증 반영)
> **분석 대상 버전.** 모바일 `main@40f34bc` (2026-05-29) / 데스크탑 v3.41.x
>
> **rev.2 변경 요지.** "데스크탑 본체를 건드리지 않는다"를 최우선 제약으로 승격(§0.1).
> 실측 검증 결과 과제 A·C는 데스크탑 영향이 0으로 확인되었고, 과제 B는 데스크탑 코드
> 0줄로 처리하는 B-γ 안을 신설하여 권고안으로 바꿨다(§3.2, §3.5′).
> 원안의 데스크탑 변경 항목(M15·M16, §2.6, §4.7)은 전부 보류로 격하했다.
> 브랜치 관련 지적(§1.5, §6-2)은 오판이었으므로 정정했다.
...
| 과제 | 난이도 | 데스크탑 영향 | 이유 |
|------|--------|--------------|------|
| **A. 주간보고 열람** | 낮음 | **없음** | 데이터는 **이미 원격 저장소에 올라가 있다.** 모바일에 읽는 UI가 없을 뿐이다. 모바일 단독 작업. |
| **B. 슬레이트 왕복** | 높음 | **있음 — §0.1 참조** | 데스크탑 쪽 소비자가 **아예 없다.** 모바일이 쓴 파일은 저장소에 도착만 하고 아무도 읽지 않는다. |
| **C. 테마 이식** | 중간 | **없음** | 양쪽 토큰 체계가 다르다(`--yk-*` vs `--color-*`). 순수 CSS 매핑 + 파생 규칙 문제. 모바일 단독 작업. |

권고 순서는 **A → C → B**다. A는 반나절이면 눈에 보이는 성과가 나고, C는 A와 독립이며,
B만 데스크탑을 건드리는 문제를 안고 있다.

---

## 0.1 데스크탑 무영향 원칙 (최우선 제약)

**전제.** 데스크탑 Workwiki는 안정 가동 중인 본체다. 모바일 개선 때문에
데스크탑 코드·동작·사용자 부담이 늘어나는 것을 허용하지 않는다.

### 검증 결과 — 실제로 확인한 것

| 검증 항목 | 결과 |
|---|---|
| 원격 기본 브랜치 | **`master`** (`ls-remote --symref` 확인). 모바일 `getLatestCommitSha` 기본값이 이미 일치 |
| 원격에 주간보고 JSON 존재 | **존재.** `origin/master`에 `reports/*.json` 6건 확인 |
| 원격에 inbox 존재 | **존재.** 모바일이 보낸 2건 모두 추적·푸시됨 |
| `.gitignore`가 `reports/`·`inbox/` 제외? | **제외 안 함.** 둘 다 정상 추적 |
| 데스크탑이 `inbox/`를 스캔? | **안 함.** `search-service`·`knowledge-web-service`는 `journals/`만, `wiki-service`는 `wiki/`만 재귀 스캔 |
| 모바일이 원격에 파일을 추가하면 데스크탑 push가 깨지나? | **안 깨짐.** `commitAndPush()`가 non-fast-forward를 감지해 `pullRebase()` 후 자동 재시도 (v3.25.1). 이미 이 경로로 inbox 1건이 들어왔다 |

### 결론

- **과제 A와 C는 데스크탑 코드·데이터·동작을 단 한 곳도 바꾸지 않는다.** 모바일은 읽기만 하고,
  읽는 대상은 이미 원격에 있으며, 데스크탑은 모바일의 존재를 알 필요조차 없다.
- **과제 B만 데스크탑을 건드린다.** 이 문서는 B에 대해 **데스크탑 변경량이 다른 세 가지 선택지**를
  §3.2에 제시하고, 무영향을 원할 경우의 권고안을 명시한다.
- 이 원칙에 따라 §2.6(`reports/index.json` 생성)과 §4.7(팔레트 단일 소스화)은
  **원안에서 격하하여 보류**한다. 둘 다 데스크탑 코드 수정을 요구하며, 없어도 모바일이 정상 동작한다.

### 부수 확인 사항 (개발과 무관하나 알아둘 것)

원격 최신 커밋이 **2026-07-03**이고 로컬 저널도 7/2가 마지막이다. 이후 데스크탑 실행 흔적이 없다.
모바일이 보는 데이터는 그 시점 스냅샷이다. 데이터가 원격에 올라가는 유일한 경로가
데스크탑 실행 중의 자동 푸시이므로, **데스크탑을 켜지 않으면 모바일도 최신 내용을 볼 수 없다.**
이는 코드 문제가 아니라 구조적 특성이다.
...
`GitHubClient.getLatestCommitSha(branch = 'master')`의 기본값은 **정확하다.**
데이터 저장소의 원격 HEAD가 실제로 `refs/heads/master`임을 확인했다(`ls-remote --symref`).
`main`으로 바꾸면 오히려 위키 트리 전체가 조용히 실패한다. **건드리지 말 것.**
장기적으로는 `/repos/{repo}`의 `default_branch`를 읽어 결정하는 편이 안전하지만, 지금은 불필요하다.
...
### 2.6 (보류) 데스크탑 측 선택 개선 — §0.1 원칙에 따라 하지 않는다

> **결정: 보류.** 아래는 데스크탑 코드 수정을 요구하므로 무영향 원칙에 어긋난다.
> 모바일은 디렉토리 스캔만으로 충분히 동작하며(API 1~2회), 이 최적화가 없어도 SC-43을 만족한다.
> 기록만 남기고 구현하지 않는다.
...
### 3.2 설계 갈림길 — 세 가지 선택지와 데스크탑 영향

§0.1의 무영향 원칙 때문에 이 절이 이 문서에서 가장 중요하다. 선택지는 셋이다.

| | **B-α** 모바일이 `journals/**.json` 직접 수정 | **B-β** inbox 경유 + 데스크탑 신규 서비스가 승격 | **B-γ** inbox 경유 + 데스크탑 Claude Chat이 승격 |
|---|---|---|---|
| **데스크탑 코드 변경** | **0줄** | 서비스 1 + 패널 1 + 자동동기화 개편 (약 2일) | **0줄** |
| 데스크탑 데이터 변경 | 없음 | 없음 | `AGENTS.md`에 절차 문단 추가 |
| 반영 시점 | 즉시 | 데스크탑 실행 + 감지 후 클릭 | 데스크탑에서 한 마디 지시할 때 |
| 사용자 조작 | 없음 | 패널에서 클릭 1회 | 채팅에 한 줄 입력 |
| 충돌 위험 | **높음** — §3.2.1 | 없음 (새 파일만 생성) | 없음 |
| 데이터 손상 가능성 | **있음** | 없음 | 낮음 (LLM이 파일을 고치므로 0은 아님) |
| 기존 원칙 | SPEC v5 §3 · CLAUDE.md §6.1 **명시적 금지 위반** | 유지 | 유지 |
| 마크다운·위키·메타 갱신 | 못 함 | 기존 파이프라인 그대로 | 기존 파이프라인 그대로 |

#### 3.2.1 왜 B-α는 "데스크탑 코드 0줄"인데도 무영향이 아닌가

B-α는 얼핏 무영향 원칙에 가장 잘 맞아 보인다. 데스크탑 소스를 한 줄도 안 고치기 때문이다.
그러나 **영향을 코드가 아니라 사용자에게 전가**한다.

1. 데스크탑 `saveJournalV3()`는 날짜 파일을 **통째로 덮어쓴다.** 병합하지 않는다.
2. 데스크탑이 오프라인/미실행 상태에서 같은 날 저널을 수정한 뒤 나중에 푸시하면,
   같은 파일을 양쪽이 고친 상태가 되어 `pullRebase()`가 **충돌로 abort**한다.
   그 순간 사용자는 데이터 폴더에서 직접 Git 충돌을 해결해야 한다.
3. 최악의 경우 하루치 슬레이트가 통째로 사라진다.

GitHub Contents API의 `sha` 조건부 쓰기로 원격 상의 경쟁은 막을 수 있지만,
**데스크탑 로컬 워킹트리와의 경쟁은 막을 방법이 없다.** 이것이 B-α를 배제하는 이유다.
"코드를 안 고친다"와 "영향이 없다"는 다른 말이다.

#### 3.2.2 결정

- **데스크탑 무영향이 최우선이라면 → B-γ를 택한다.** 데스크탑 소스 0줄, 충돌 위험 없음,
  대가는 "채팅에 한 줄 지시" 뿐이다. 상세는 §3.5′.
- **나중에 완전 자동화를 원하게 되면 → B-β로 승격한다.** B-γ에서 만든 inbox 스키마 v2가
  그대로 B-β의 입력이 되므로 **모바일 쪽 작업은 하나도 버려지지 않는다.**
- **B-α는 채택하지 않는다.**

즉 이 문서의 권고는 **모바일만 만들고(§3.3~3.4), 데스크탑은 당분간 손대지 않는다(B-γ)**이다.
§3.5·§3.6은 나중에 B-β로 갈 때를 위한 설계이며, 지금 구현 대상이 아니다.
...
### 3.5′ B-γ — 데스크탑 코드 0줄로 승격하기 (권고안)

데스크탑에는 이미 **데이터 폴더를 작업 디렉토리로 하는 Claude 터미널**이 붙어 있다.
`claude-chat-service.ts`가 PTY를 띄우며 `cwd = getDataRoot()`로 설정하고,
`CLAUDE.md`·`AGENTS.md`를 컨텍스트로 자동 로드한다.

즉 **저널 JSON을 읽고 고칠 수 있는 에이전트가 데스크탑 안에 이미 존재한다.** 새로 만들 것이 없다.

**필요한 것은 절차서 한 문단뿐이다.** 데이터 폴더의 `AGENTS.md`에 다음을 추가한다.
(이것은 데스크탑 *앱*의 변경이 아니라 *데이터*의 변경이며, 앱 재빌드가 필요 없다.)
...
1. `inbox/` 아래 `_processed/`를 제외한 `.md` 파일을 최신순으로 나열하고 프론트매터를 읽는다.
2. 각 항목에 대해:
   - `kind: append` → `target_slate_id`(또는 제목)로 대상 슬레이트를 찾아 본문을 이어붙인다.
   - 그 외 → `target_date`의 `journals/YYYY/MM/DD.json`을 읽고, `slates` 배열 **끝에** 새 항목을 추가한다.
     `id`는 `slate-{타임스탬프}`, `createdAt`/`updatedAt`은 **프론트매터의 `created_at`을 그대로 쓴다.**
3. 저널 파일은 **읽고 → 배열에 추가 → 다시 쓴다.** 기존 슬레이트를 삭제하거나 순서를 바꾸지 않는다.
4. 처리한 원본을 `inbox/_processed/YYYY/MM/` 으로 이동한다.
5. `inbox/_state.json`에 처리 기록 한 줄을 추가한다(형식은 §3.6 참조).
6. 결과를 표로 보고한다: 파일명 / 귀속 날짜 / 슬레이트 제목 / 조치.

**금지.** 마크다운 생성·위키 ingest·메타 인덱스 갱신은 하지 않는다. 사용자가 앱에서 직접 수행한다.
```

**동작 방식.** 사용자가 데스크탑에서 Claude Chat을 열고 "inbox 정리해줘"라고 입력한다. 끝이다.
앱은 그 결과로 바뀐 저널 파일을 평소처럼 읽고, 자동 푸시가 평소처럼 원격에 올린다.
**앱은 자기 데이터 폴더의 파일이 왜 늘었는지 알 필요가 없다.**

**장점**
- 데스크탑 소스 변경 0줄, 재빌드 0회, 회귀 위험 0.
- 승격 규칙을 바꾸고 싶으면 `AGENTS.md` 문단만 고치면 된다. 배포가 필요 없다.
- LLM이 처리하므로 "이건 어제 회의 내용이니 6/26에 붙여줘" 같은 예외 지시가 그대로 통한다.

**한계와 완화**
- 자동이 아니다. 사용자가 지시해야 한다. → 모바일 '보낸 메모'에 미처리 건수를 표시해
  "정리할 게 3건 있음"을 폰에서 인지할 수 있게 한다(§3.7).
- LLM이 JSON을 잘못 고칠 위험이 0은 아니다. → 절차서에 "기존 슬레이트 삭제 금지"를 명시했고,
  데이터 폴더가 Git 저장소이므로 **잘못되면 되돌릴 수 있다.** 이것이 실질적 안전망이다.
- 여러 건을 한 번에 처리할 때 토큰을 쓴다. → 구독 내 CLI 호출이라 추가 과금은 없다.

### 3.5 (보류) B-β — 데스크탑 신규 서비스로 자동화

> **결정: 지금은 구현하지 않는다.** §0.1 무영향 원칙에 따라 B-γ를 먼저 쓴다.
> 아래는 나중에 자동화를 원할 때를 위한 설계이며, B-γ에서 만든 스키마·상태 파일과 완전히 호환된다.
...
> 이 절은 **B-γ와 B-β에 공통으로 적용된다.** B-γ에서는 §3.5′의 절차서에 따라 Claude가,
> B-β에서는 InboxService가 같은 규칙으로 파일을 옮기고 상태를 기록한다.
> 규칙이 같으므로 나중에 B-β로 옮겨가도 기존 기록이 그대로 유효하다.
...
- **SC-45′.** 데스크탑 Claude Chat에 "inbox 정리해줘" 한 줄로 승격이 수행된다. **앱 재빌드 없이 동작한다.**
- **SC-46′.** 승격 후 `git diff`로 확인했을 때 기존 슬레이트가 하나도 삭제·변형되지 않는다.
- **SC-47′.** 승격된 슬레이트의 작성 시각이 **모바일 작성 시각**으로 보존된다.
- **SC-48′.** 처리된 원본이 `inbox/_processed/`로 이동하여 다음 정리에서 다시 잡히지 않는다.
...
- **SC-51a.** M12~M14.5 완료 후에도 데스크탑 앱 소스에 변경이 **0줄**이다.

**B-β 기준 (보류. 나중에 자동화할 때)**

- **SC-45.** 데스크탑이 앱 시작 5분 이내에 새 inbox 항목을 감지하고 배지를 띄운다.
- **SC-46.** 승격 시 해당 날짜 저널에 슬레이트가 추가되고, 기존 슬레이트가 유실되지 않는다.
- **SC-47 / SC-48.** SC-47′ / SC-48′과 동일하되 서비스가 자동 수행한다.
...
### 4.7 (보류) 장기 — 팔레트 단일 소스화

> **결정: 보류.** §0.1 무영향 원칙에 어긋난다(데스크탑이 `config/themes.json`을 읽도록 고쳐야 함).
> 모바일이 자기 팔레트를 하드코딩해도 아무 문제가 없다. 테마 색은 자주 바뀌는 값이 아니다.
> 데스크탑 팔레트가 바뀌면 그때 모바일 `palettes.ts`를 손으로 맞추면 된다.
...
| M | 범위 | 저장소 | 데스크탑 영향 | 완료 조건 | 공수 |
|---|------|--------|---------------|-----------|------|
| **M12** | ReportsService + ReportsView + 홈 진입점 | 모바일 | **없음** | SC-38~43 | 0.5일 |
| **M13** | 테마 8종 이식 + 생성 스크립트 + FOUC/상태바 | 모바일 | **없음** | SC-52~57 | 1일 |
| **M14** | inbox 스키마 v2 + QuickMemoSheet 확장 + InboxSentView | 모바일 | **없음** | SC-44, 49, 51 | 1일 |
| **M14.5** | 데이터 폴더 `AGENTS.md`에 승격 절차 추가 (B-γ) | 데이터 | **앱 변경 없음** | SC-45′~48′ | 0.5시간 |
| ~~M15~~ | ~~InboxService + InboxPanel + auto-sync(pull)~~ | **데스크탑** | **있음 — 보류** | SC-45~48, 50 | 2일 |
| ~~M16~~ | ~~reports/index.json + 팔레트 단일 소스화~~ | 양쪽 | **있음 — 보류** | — | — |

**M15·M16은 무영향 원칙에 따라 보류한다.** 필요해지면 그때 별도 판단한다.

**의존성.** M12·M13·M14는 서로 완전히 독립이다. M14.5는 M14 이후에 하면 되고,
안 해도 모바일 쪽은 정상 동작한다(파일이 inbox에 쌓일 뿐 손실 없음).

**M12~M14.5를 다 해도 데스크탑 앱은 한 줄도 바뀌지 않고, 재빌드도 필요 없다.**

**중간 점검.** M12 완료 후(주간보고 렌더링 방향 확인), M13 완료 후(8종 실기기 육안 확인),
M14.5 완료 후(승격 1회 실행하여 저널 무결성 확인 — Git diff로 기존 슬레이트 보존 검증).

---

## 6. 리스크와 미해결 쟁점

1. ~~자동 pull이 로컬 작업을 방해할 위험.~~ **해소됨.** B-γ를 택하면 자동 pull을 도입하지 않는다.
   기존 `commitAndPush()`의 non-fast-forward 자동 rebase만으로 충분하며, 이는 이미 검증된 경로다.
2. ~~`getLatestCommitSha`의 기본 브랜치 문제.~~ **오판이었다.** 원격 HEAD가 실제로 `master`이므로
   현재 기본값이 정확하다(§1.5). **고치지 말 것.**
...
**데스크탑은 안 건드림.** 과제 A(주간보고)와 C(테마)는 데스크탑 코드·데이터·동작 영향이 **0**임을
실측으로 확인함. B만 문제였는데, 데스크탑의 Claude 터미널(cwd=데이터폴더)을 쓰는 B-γ로 우회함.
데스크탑 앱 소스 0줄, 재빌드 0회로 M12~M14.5 전부 가능함.

주간보고는 **이미 원격에 있음.** `origin/master`에 `reports/*.json` 6건 실물 확인함.
`wiki/notes/weekly-*.md`로도 이중 저장됨. 모바일에 읽는 코드가 없을 뿐임. 가장 싸고 효과 큼. 먼저 할 것.

원격 기본 브랜치는 **`master`가 맞음.** 모바일 기본값이 이미 정확하니 건드리지 말 것.
v1 초안에서 이걸 리스크로 적었던 건 오판이었음.

모바일이 쓴 inbox 메모를 데스크탑이 읽는 코드는 **한 줄도 없음.** 저장소 전체 검색 0건임.
파일 2건이 이미 도착해 방치돼 있음. 배관은 뚫려 있고 수신부만 없음.

자동 동기화가 **push 단방향**임. 다만 `commitAndPush()`가 non-fast-forward를 감지하면
자동으로 `pull --rebase` 후 재시도하므로, 모바일이 올린 파일은 데스크탑이 다음 푸시 때 흡수함.
실제로 inbox 2건이 이 경로로 들어와 있음. **따라서 auto-pull 개편은 필수가 아님.**
v1 초안에서 "pull 없으면 B가 동작 안 함"이라 단정했으나, B-γ에선 해당 없음.

모바일이 저널을 직접 쓰는 안(B-α)은 채택 안 함. "데스크탑 코드 0줄"이라 무영향처럼 보이지만,
충돌 해결 부담을 사용자에게 떠넘기고 하루치 슬레이트 통째 유실 시나리오가 실재함.
**코드를 안 고치는 것과 영향이 없는 것은 다름.**
